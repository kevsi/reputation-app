  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err?.status === 409) {
        ...
        setError('Le mot de passe est trop faible. Il doit contenir au moins 8 caractères, une majuscule et un caractère spécial.');
        ...
        <button 
          ...
          onClick={async () => {
            ...
            try {
              ...
              const errorMsg = err?.message || 'Erreur lors de la connexion Google';
              setError(errorMsg);
            } finally {
          ...
          className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 text-slate-700 dark:text-slate-300 font-bold py-3 rounded-2xl transition-all flex items-center justify-center gap-3 active:scale-[0.98] disabled:opacity-70"
        >
          {isGoogleLoading ? (
            <Loader2 className="w-5 h-5 animate-spin" />
          ) : (
