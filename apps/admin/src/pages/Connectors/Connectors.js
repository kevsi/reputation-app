...
exports.default = ConnectorsPage;
const react_1 = require("react");
const ConnectorCard_1 = require("@/components/connectors/ConnectorCard");
...
function ConnectorsPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [sourcesRes, statusRes] = await Promise.all([
                ...
                api_client_1.apiClient.getSystemStatus()
            ]);
            // Handle Sources
            const sourceData = sourcesRes.data || (Array.isArray(sourcesRes) ? sourcesRes : []);
            const mappedSources = sourceData.map((source) => ({
                id: source.id,
                ...
                lastSync: source.lastScrape ? (0, date_fns_1.formatDistanceToNow)(new Date(source.lastScrape), { addSuffix: true, locale: locale_1.fr }) : "Jamais"
            }));
    ...
    const handleConfigure = async (connectorId) => {
        ...
        try {
            await api_client_1.apiClient.updateSource(connectorId, { isActive: newActive });
            // Refresh data
            await fetchData();
        }
        catch (error) {
    ...
    const getIconForType = (type) => {
        ...
        if (t.includes('youtube'))
            return "📹";
        if (t.includes('news') || t.includes('web'))
            return "📰";
        return "🌐";
    };
    ...
    return (<div className="p-4 sm:p-6 md:p-8 max-w-[1600px] mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={fetchData} className="flex items-center gap-2 px-4 py-2 bg-foreground text-background rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">
      ...
      {loading && !systemStatus ? (<div className="flex justify-center py-12">
          ...
          <h2 className="text-lg font-semibold mb-4 text-foreground flex items-center gap-2">
            <lucide_react_1.Server className="w-5 h-5 text-blue-500"/>
            Sources de Données
          </h2>

          {sources.length === 0 ? (<div className="text-center py-12 bg-muted/30 rounded-xl border border-dashed border-border">
              ...
              {sources.map((connector) => (<ConnectorCard_1.ConnectorCard key={connector.id} name={connector.name} icon={connector.icon} status={connector.status} collected24h={connector.collected24h} errors={connector.errors} lastSync={connector.lastSync} onConfigure={() => handleConfigure(connector.id)} onTest={() => handleTest(connector.id)}/>))}
            </div>)}
        </>)}
    </div>);
}
