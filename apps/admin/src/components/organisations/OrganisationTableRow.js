...
exports.OrganisationTableRow = OrganisationTableRow;
const badge_1 = require("@/components/ui/badge");
const button_1 = require("@/components/ui/button");
function OrganisationTableRow({ name, createdDate, plan, users, brands, status, onViewDetails }) {
    const getPlanStyle = () => {
    ...
    return (<tr className="group hover:bg-muted/30 transition-colors">
      ...
      {/* Plan */}
      <td className="px-6 py-4">
        <badge_1.Badge className={`font-bold text-[10px] uppercase tracking-wider ${getPlanStyle()}`}>
          {plan}
        </badge_1.Badge>
      </td>
      ...
      {/* Statut */}
      <td className="px-6 py-4">
        <badge_1.Badge variant="outline" className={`font-medium text-[11px] ${getStatusStyle()}`}>
          {status}
        </badge_1.Badge>
      </td>
      ...
      <td className="px-6 py-4 text-right">
        <button_1.Button variant="ghost" size="sm" onClick={onViewDetails} className="h-8 text-xs font-semibold hover:bg-primary/5 hover:text-primary">
          Gérer
        </button_1.Button>
      </td>
    </tr>);
}
exports.default = OrganisationTableRow;
