...
function UserTableRow({ name, email, organisationName, role, createdAt, status, onManage }) {
    const getRoleStyle = () => {
    ...
    return (<tr className="group hover:bg-muted/30 transition-colors">
      ...
      {/* Rôle */}
      <td className="px-6 py-4">
        <badge_1.Badge className={`font-bold text-[10px] uppercase tracking-wider ${getRoleStyle()}`}>
          {role}
        </badge_1.Badge>
      </td>
      ...
      <td className="px-6 py-4 text-right">
        <button_1.Button variant="ghost" size="sm" onClick={onManage} className="h-8 text-xs font-semibold hover:bg-primary/5 hover:text-primary">
          Gérer
        </button_1.Button>
      </td>
    </tr>);
